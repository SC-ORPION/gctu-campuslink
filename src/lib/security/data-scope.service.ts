...
// ---------------------------------------------------------------------------
// Admin Role Type
// Defined here as the canonical source; re-exported for rbac.service.ts
// to import if needed. Values align with CampusLink's role hierarchy.
// ---------------------------------------------------------------------------
import { AdminRole } from './rbac.service';

// ---------------------------------------------------------------------------
// Hostel Assignment Interface
// Tracks which hostels an admin is permitted to manage. Stored in-memory
// via globalThis so that assignments survive HMR reloads in development
// but are intentionally non-persistent (authoritative source is the DB
// admin record or an external IAM in production).
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
const globalScopeRegistry = globalThis as unknown as {
...
export class DataScopeService {
   ...
   */
  static registerAdminRole(adminId: string, role: AdminRole): void {
   ...
   */
  static getAdminRole(adminId: string): AdminRole {
   ...
   */
  static getAssignments(): HostelAssignment[] {
   ...
   */
  static async assignHostelScope(
    superAdminId: string,
    ...
    if (callerRole !== 'SUPER_ADMIN') {
      throw new Error(
        `Access Denied: Only SUPER_ADMIN can assign hostel scopes. ` +
    ...
    const assignment: HostelAssignment = {
   ...
   */
  static getAdminHostelScope(adminId: string): string[] {
    ...
    const assignment = globalScopeRegistry.hostelAssignments.find(
      (a) => a.adminId === adminId,
    );
   ...
   * ```
   */
  static filterStudentsByScope(adminId: string, students: any[]): any[] {
    ...
    return students.filter((student) => {
      ...
      const bookings: any[] = student.bookings ?? [];
      return bookings.some((b: any) => scopeSet.has(b.hostelId));
    });
  }
   ...
   * ```
   */
  static filterPaymentsByScope(adminId: string, payments: any[]): any[] {
    const scope = this.getAdminHostelScope(adminId);
    ...
    return payments.filter((payment) => {
      const hostelId: string | undefined = payment.booking?.hostelId;
   ...
   */
  static assertHostelAccess(adminId: string, hostelId: string): void {
    const scope = this.getAdminHostelScope(adminId);
    ...
    if (!scope.includes(hostelId)) {
      throw new Error(
        `Access Denied: Admin ${adminId} does not have management access to hostel ${hostelId}. ` +
...
export class SystemIntegrityProtection {
  // -----------------------------------------------------------------------
   ...
   */
  static async preventActiveAllocationDeletion(allocationId: string): Promise<void> {
    ...
    if (!allocation) {
      throw new Error(
        `Integrity Error: Allocation ${allocationId} does not exist.`,
      );
    }

    if (allocation.revokedAt === null) {
      throw new Error(
        'Cannot delete active allocation. Must revoke first.',
      );
    }
  }
   ...
   */
  static async preventVerifiedPaymentModification(
    paymentId: string,
    ...
    if (!payment) {
      throw new Error(
        `Integrity Error: Payment ${paymentId} does not exist.`,
      );
    }

    if (payment.status === 'VERIFIED' && !hasOverridePermission) {
      throw new Error(
        `Integrity Error: Payment ${paymentId} has already been VERIFIED. ` +
   ...
   */
  static validateStateTransitionIntegrity(
    currentState: string,
    targetState: string,
    ...
    if (!allowedTargets) {
      throw new Error(
        `Invalid State Transition: Current state "${currentState}" is not recognised ` +
   ...
   */
  static preventDirectDbMutation(operation: string): void {
    const timestamp = new Date().toISOString();
    console.warn(
      `[INTEGRITY_WARNING] [${timestamp}] Direct DB mutation detected: "${operation}". ` +
  ...
  static async validateEntityExists(
    ...
    if (!modelKey) {
      throw new Error(
        `Integrity Error: Unknown entity type "${entityType}". ` +
    ...
    if (!record) {
      throw new Error(
        `Entity Not Found: ${entityType} with ID "${entityId}" does not exist.`,
      );
    }
  }
}
