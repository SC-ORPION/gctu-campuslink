...
exports.SystemLockdownService = void 0;
const db_1 = require("../db");
const audit_log_1 = require("../audit/audit-log");
const integrity_checker_1 = require("../monitoring/integrity-checker");
// ---------------------------------------------------------------------------
// Global in-memory registries (survives HMR, reset on cold restart)
// ---------------------------------------------------------------------------
const globalLockdownRegistry = globalThis;
...
async function assertSuperAdmin(userId) {
    ...
    if (!user) {
        throw new Error(`[LOCKDOWN] Authorization failed: user ${userId} not found.`);
    }
    if (user.role !== 'admin') {
        throw new Error(`[LOCKDOWN] Access denied: user ${userId} requires SUPER_ADMIN (admin) role. Current role: ${user.role}.`);
    }
}
// ---------------------------------------------------------------------------
// SystemLockdownService
// ---------------------------------------------------------------------------
class SystemLockdownService {
    // -----------------------------------------------------------------------
     ...
     */
    static async enableSystemLockdown(adminId, reason) {
        await assertSuperAdmin(adminId);
        if (globalLockdownRegistry.__campuslink_lockdown_state?.active) {
            throw new Error('[LOCKDOWN] System is already in lockdown mode. Disable the current lockdown before re-enabling.');
        }
        const state = {
            active: true,
            reason,
        ...
        console.log(`[LOCKDOWN] System lockdown ACTIVATED by ${adminId}. Reason: ${reason}`);
        return state;
    }
    /**
     ...
     */
    static async disableSystemLockdown(adminId) {
        await assertSuperAdmin(adminId);
        const current = globalLockdownRegistry.__campuslink_lockdown_state;
        if (!current?.active) {
            throw new Error('[LOCKDOWN] System is not currently in lockdown mode.');
        }
        ...
        console.log(`[LOCKDOWN] System lockdown DEACTIVATED by ${adminId}. Previous reason: ${previousReason}`);
    }
    /**
     * Quick boolean check — is the system currently locked?
     */
    static isSystemLocked() {
        return (globalLockdownRegistry.__campuslink_lockdown_state?.active === true);
    }
    /**
     * Return full lockdown details or `null` when the system is not locked.
     */
    static getLockdownStatus() {
     ...
     */
    static async assertNotLocked(userId) {
        ...
        throw new Error(`[LOCKDOWN] System is currently in lockdown mode. Reason: ${state.reason}. ` +
     ...
     * @throws If the caller is not a SUPER_ADMIN.
     */
    static async emergencyRecoveryMode(adminId) {
        await assertSuperAdmin(adminId);
        ...
        if (!SystemLockdownService.isSystemLocked()) {
            await SystemLockdownService.enableSystemLockdown(adminId, 'EMERGENCY_RECOVERY');
        ...
        console.log(`[RECOVERY] Job queue processing PAUSED by ${adminId}.`);
        // ------------------------------------------------------------------
        // Step 3: Run integrity checker
        // ------------------------------------------------------------------
        let integrityResult;
        try {
            integrityResult = await integrity_checker_1.IntegrityChecker.runIntegrityCheck();
        }
        catch (err) {
        ...
        // ------------------------------------------------------------------
        const [bookingCount, paymentCount, allocationCount, roomCount] = await Promise.all([
            db_1.prisma.booking.count(),
            db_1.prisma.payment.count(),
            db_1.prisma.allocation.count({ where: { revokedAt: null } }),
        ...
        console.log(`[RECOVERY] Emergency recovery completed by ${adminId}. ` +
...
exports.SystemLockdownService = SystemLockdownService;
exports.default = SystemLockdownService;
