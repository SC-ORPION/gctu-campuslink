...
const ALL_PERMISSIONS: readonly Permission[] = Object.values(Permission) as Permission[];
...
export const PERMISSION_MATRIX: Readonly<Record<AdminRole, readonly Permission[]>> = {
  ...
  [AdminRole.ADMIN]: ALL_PERMISSIONS.filter((p) => p !== Permission.SYSTEM_LOCKDOWN),
...
// ============================================================================
// IN-MEMORY ROLE REGISTRY (globalThis pattern)
// ============================================================================

/**
 * In-memory role assignment registry.
 *
 * The Prisma schema's User model uses a binary `Role` enum (student | admin).
 * Since there is no `adminRole` column or JSON metadata field on the User model,
 * fine-grained RBAC roles are stored here. In production, this would be backed
 * by a dedicated `admin_roles` database table or a JSONB column on the user row.
 *
 * This follows the same globalThis singleton pattern used by:
 * - AuditLogService (audit-log.ts)
 * - SystemHealthMonitor (system-health.ts)
 * - JobQueue (job-queue.ts)
 */
...
const globalRbacRegistry = globalThis as unknown as {
  __rbac_role_assignments: Map<string, RoleAssignment>;
  __rbac_initialized: boolean;
};

if (!globalRbacRegistry.__rbac_role_assignments) {
  globalRbacRegistry.__rbac_role_assignments = new Map<string, RoleAssignment>();
}
...
export async function getUserRole(userId: string): Promise<AdminRole | null> {
  ...
  if (!user) {
    throw new RBACError(
      `RBAC lookup failed: User ${userId} does not exist.`,
      'USER_NOT_FOUND',
      userId
    );
  }
...
export async function validatePermission(
  ...
  if (!user) {
    ...
    throw new RBACError(
      `Access Denied: User ${userId} does not exist.`,
      'USER_NOT_FOUND',
      userId
    );
  }
  ...
  if (user.status !== 'ACTIVE') {
    ...
    throw new RBACError(
      `Access Denied: Account ${userId} is ${user.status}. Only ACTIVE accounts can perform actions.`,
      'ACCOUNT_INACTIVE',
      userId
    );
  }
  ...
  if (user.role !== 'admin') {
    ...
    throw new RBACError(
      `Access Denied: User ${userId} has role '${user.role}'. Admin role required.`,
      'INSUFFICIENT_ROLE',
      userId
    );
  }
  ...
  if (!adminRole) {
    ...
    throw new RBACError(
      `Access Denied: Unable to resolve admin role for user ${userId}.`,
  ...
  if (!hasPermission(adminRole, requiredPermission)) {
    ...
    throw new RBACError(
      `Access Denied: Role '${adminRole}' does not include permission '${requiredPermission}'.`,
      'PERMISSION_DENIED',
      userId
    );
  }
...
export async function assignRole(
  ...
  const requestingAdmin = await validatePermission(adminId, Permission.SYSTEM_LOCKDOWN);

  if (requestingAdmin.adminRole !== AdminRole.SUPER_ADMIN) {
    await auditDeniedAccess(adminId, 'ROLE_ASSIGNMENT' as Permission, 'PERMISSION_DENIED', {
      reason: 'Only SUPER_ADMIN can assign roles',
      targetUserId,
      requestedRole: newRole,
    });
    throw new RBACError(
      `Access Denied: Only SUPER_ADMIN users can assign roles. Current role: ${requestingAdmin.adminRole}.`,
      'PERMISSION_DENIED',
      adminId
    );
  }
  ...
  if (!targetUser) {
    throw new RBACError(
      `Role assignment failed: Target user ${targetUserId} does not exist.`,
      'USER_NOT_FOUND',
      targetUserId
    );
  }
  ...
  if (targetUser.role !== 'admin') {
    throw new RBACError(
      `Role assignment failed: Target user ${targetUserId} must have base 'admin' role. Current: '${targetUser.role}'.`,
      'INSUFFICIENT_ROLE',
      targetUserId
    );
  }
  ...
  if (adminId === targetUserId && newRole !== AdminRole.SUPER_ADMIN) {
    throw new RBACError(
      `Role assignment failed: SUPER_ADMIN cannot demote themselves. This prevents platform lockout.`,
  ...
  const previousRole = previousAssignment?.role ?? AdminRole.ADMIN;
  ...
  console.log(
    `[RBAC] Role assigned: ${targetUser.email} (${targetUserId}) -> ${newRole} by ${adminId}`
  );
 ...
 */
export function getAllRoleAssignments(): RoleAssignment[] {
 ...
 */
export async function revokeRole(adminId: string, targetUserId: string): Promise<boolean> {
  // Verify caller is SUPER_ADMIN
  const requestingAdmin = await validatePermission(adminId, Permission.SYSTEM_LOCKDOWN);

  if (requestingAdmin.adminRole !== AdminRole.SUPER_ADMIN) {
    throw new RBACError(
      `Access Denied: Only SUPER_ADMIN users can revoke roles.`,
      'PERMISSION_DENIED',
      adminId
    );
  }
  ...
  if (adminId === targetUserId) {
    throw new RBACError(
      `Role revocation failed: SUPER_ADMIN cannot revoke their own role.`,
      'SELF_DEMOTION_BLOCKED',
      adminId
    );
  }
  ...
  if (removed && previousAssignment) {
    await AuditLogService.logAudit({
      ...
      newState: AdminRole.ADMIN,
      actor: 'admin',
      actorId: adminId,
      metadata: {
        previousRole: previousAssignment.role,
        revertedTo: AdminRole.ADMIN,
        reason: 'Manual revocation by SUPER_ADMIN',
      },
    });

    console.log(
      `[RBAC] Role revoked: ${targetUserId} reverted from ${previousAssignment.role} to ${AdminRole.ADMIN} by ${adminId}`
    );
  }
 ...
 */
export async function seedSuperAdmin(userId: string): Promise<RoleAssignment> {
  // Check that no SUPER_ADMIN already exists
  const existingSuperAdmins = Array.from(
    globalRbacRegistry.__rbac_role_assignments.values()
  ).filter((a) => a.role === AdminRole.SUPER_ADMIN);

  if (existingSuperAdmins.length > 0) {
    throw new RBACError(
      `SUPER_ADMIN seed failed: A SUPER_ADMIN already exists (${existingSuperAdmins[0].userId}). Use assignRole() instead.`,
      'SEED_CONFLICT',
      userId
    );
  }
  ...
  if (!user) {
    throw new RBACError(
      `SUPER_ADMIN seed failed: User ${userId} does not exist.`,
      'USER_NOT_FOUND',
      userId
    );
  }
  ...
  if (user.role !== 'admin') {
    throw new RBACError(
      `SUPER_ADMIN seed failed: User ${userId} must have base 'admin' role. Current: '${user.role}'.`,
      'INSUFFICIENT_ROLE',
      userId
    );
  }
  ...
  await AuditLogService.logAudit({
    ...
    previousState: 'NONE',
    newState: AdminRole.SUPER_ADMIN,
    actor: 'system',
    metadata: {
      email: user.email,
      fullName: user.fullName,
...
/**
 * Represents a fully validated admin user returned by `validatePermission()`.
 * Contains both the base Prisma fields and the resolved RBAC admin role.
 */
...
export class RBACError extends Error {
  ...
  constructor(message: string, code: RBACErrorCode, userId: string) {
    super(message);
    this.name = 'RBACError';
    this.code = code;
    this.userId = userId;
