...
export default function HostelDetailClient({ hostel }: HostelDetailClientProps) {
  ...
  return (
    <div className="bg-white min-h-screen">
      {/* Top Navigation */}
      <div className="sticky top-0 z-40 bg-white/80 backdrop-blur-md border-b border-[#E2E8F0]">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          <Link href="/student/hostels" className="flex items-center gap-2 text-[#64748B] hover:text-[#1D4ED8] transition-colors font-bold text-sm">
            <ArrowLeft size={20} />
            <span>Back to Listings</span>
          </Link>
          <div className="flex items-center gap-4">
             <button className="hidden md:flex items-center gap-2 px-4 py-2 text-sm font-bold text-[#64748B] hover:text-[#DC2626] transition-colors">
      ...
      <div className="container mx-auto px-4 py-8">
              ...
              <div className="premium-card p-8">
                <div className="flex items-center gap-2 mb-4">
                  <span className="status-badge success">
                    <ShieldCheck size={12} className="mr-1 inline-block" /> Verified Listing
                  </span>
                </div>

                <h1 className="text-4xl font-bold text-[#0F172A] mb-2">{hostel.name}</h1>
                
                <div className="flex items-center gap-2 text-[#64748B] mb-8">
                  <MapPin size={18} className="text-[#1D4ED8]" />
                  <span className="font-semibold">{hostel.location_name}</span>
                </div>

                <div className="grid grid-cols-2 gap-4 mb-8">
                  <div className="bg-[#F8FAFC] border border-[#E2E8F0] p-4 rounded-xl text-center">
                    <div className="text-[10px] font-bold text-[#64748B] uppercase mb-1">Campus</div>
                    <div className="text-sm font-bold text-[#0F172A]">{hostel.campus}</div>
                  </div>
                  <div className="bg-[#F8FAFC] border border-[#E2E8F0] p-4 rounded-xl text-center">
                    <div className="text-[10px] font-bold text-[#64748B] uppercase mb-1">Students</div>
                    <div className="text-sm font-bold text-[#0F172A] capitalize">{hostel.gender_rule.replace('_ONLY', 's').toLowerCase()}</div>
                  </div>
                </div>
                ...
                <div className="space-y-4 mb-8">
                  <a 
                    ...
                    className="flex items-center justify-center gap-3 w-full bg-[#059669] text-white py-4 rounded-xl font-bold shadow-sm hover:-translate-y-0.5 transition-all"
                  >
                ...
                <div className="text-center">
                  <button className="text-xs font-bold text-[#94A3B8] hover:text-[#DC2626] transition-colors uppercase tracking-widest">
        ...
        <section className="mt-24 border-t border-gray-100 pt-16">
          <div className="mb-12">
            <h2 className="text-3xl font-bold mb-2 flex items-center gap-4 text-[#0F172A]">
              <Building2 className="text-[#1D4ED8]" /> Room Variants & Pricing
            </h2>
            <p className="text-[#64748B] font-medium">Select the room type that best fits your budget and lifestyle.</p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {hostel.rooms?.map((room) => (
              <div key={room.id} className="group bg-white p-8 rounded-2xl border border-[#E2E8F0] shadow-sm hover:shadow-md hover:border-[#CBD5E1] transition-all">
                <div className="flex justify-between items-start mb-8">
                  <h3 className="text-xl font-bold text-[#0F172A]">{room.room_number || "Standard Room"}</h3>
                  <div className="text-right">
                    <div className="text-2xl font-bold text-[#1D4ED8]">GH₵{room.price}</div>
                    <div className="text-[10px] font-bold text-[#64748B] uppercase tracking-wider">Yearly</div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 mb-8">
                  ...
                  <div className="flex items-center gap-3 text-[#475569]">
                    <div className="w-8 h-8 bg-[#F8FAFC] border border-[#E2E8F0] rounded-lg flex items-center justify-center">
                      <Bath size={16} className="text-[#059669]" />
                    </div>
                ...
                <div className="space-y-3 mb-8">
                  {room.ac_available && (
                    <div className="flex items-center gap-3 text-[#1D4ED8]">
                      <Wind size={16} />
                      <span className="text-xs font-bold uppercase tracking-wide">Air Conditioning</span>
                    </div>
                  )}
                  {room.wifi_available && (
                    <div className="flex items-center gap-3 text-[#7C3AED]">
                      <Wifi size={16} />
                      <span className="text-xs font-bold uppercase tracking-wide">High-speed WiFi</span>
                    </div>
                  )}
                  {room.kitchen_available && (
                    <div className="flex items-center gap-3 text-[#059669]">
                      <Utensils size={16} />
                      <span className="text-xs font-bold uppercase tracking-wide">Kitchen Access</span>
                    </div>
                  )}
                </div>
                ...
                <button 
                  onClick={() => setShowConfirmModal(true)}
                  className="btn btn-secondary w-full group-hover:btn-primary"
                >
                  Select Room
                </button>
              </div>
      ...
      {showConfirmModal && (
        <div className="fixed inset-0 bg-[#0F172A]/80 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="premium-card max-w-md w-full p-8 shadow-2xl space-y-6">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-full bg-[#FFFBEB] border border-[#FEF3C7] flex items-center justify-center text-[#D97706]">
                <AlertTriangle size={24} />
              </div>
              <div>
                <h3 className="text-lg font-bold text-[#0F172A]">Confirm Selection</h3>
                <p className="text-sm font-semibold text-[#64748B]">Locking hostel reservation</p>
              </div>
            </div>

            <div className="bg-[#FFFBEB] border border-[#FEF3C7] p-5 rounded-xl text-sm font-semibold text-[#B45309] leading-relaxed">
            ...
            <div className="grid grid-cols-2 gap-4 pt-4">
              ...
              <button 
                ...
                disabled={bookingLoading}
              >
                {bookingLoading ? <Loader2 size={16} className="animate-spin mx-auto" /> : 'Confirm Selection'}
              </button>
            </div>
          </div>
        </div>
